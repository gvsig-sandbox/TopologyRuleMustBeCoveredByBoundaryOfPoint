...
class MarkPointAction(AbstractTopologyRuleAction):
    ...
    def execute(self, rule, line, parameters):
        try:
            ...
            errorsLayer.edit()
            errorsLayer.append(GEOMETRY=line.getFeature1().getFeature().getDefaultGeometry())
            errorsLayer.commit()
    ...
    def checkErrorsLayerName(self):
        if self.errorsLayerName == "":
            self.errorsLayerName = "MustBeCoveredByBoundaryOf_" + datetime.now().strftime("%Y%m%d%H%M%S")
